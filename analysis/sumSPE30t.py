...
date_code = sys.argv[1]  # YYMMDDTAABB 형식
date = date_code[:6]  # YYMMDD
dateString = datetime.strptime(date, '%y%m%d').strftime('%d %b %Y')

allRootFiles = os.listdir(directory)
inFiles = list(filter(lambda file: date_code in file, allRootFiles))  # YYMMDDTAABB 기준으로 필터링
...
for root in inFiles:
    ...
    try:
        ...
        alphaEvents=rootFileOpen.arrays(filter_name='adc_b4_ch13',library='np')['adc_b4_ch13']
    ...
    for channel in channels:
        ...
        try: #only add processed events to hist if all channels are OK
            ...
            '''
            for event in events:
                max_idx = np.argmax(event)  # Peak 위치 찾기
                left_idx = max(0, max_idx - 10)  # 왼쪽 10개 값
                right_idx = min(len(event), max_idx + 20)  # 오른쪽 20개 값
                regionOfInterest = event[left_idx:right_idx]  # 고정된 ROI 선택
...
p0={channel:[1,0.4,0.5e5] for channel in channels}
...
for n,channel in enumerate(channels):
    ...
    plt.xlim(-0.5, 6.5)
    plt.savefig('diagnostics/30t/temp/'+date+'diagnosticplot.png')
    try:
        ...
        print(f'fit didn\'t work for this day! adjust parameters maybe? -> {e}')
        exit(0)
...
#pd.DataFrame(results).transpose().to_csv('/media/disk_k/30t-DATA/csv/phase0/non-validated/bnl30t_spe_fit_results_'+date+'.csv',index=False)
