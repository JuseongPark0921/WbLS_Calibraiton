...
for n,channel in enumerate(channels):
    try:
        ...
        groupSumHistograms[channel]=groupSumHistograms[channel]/spemean
        ...
        plt.xlim(-0.5, 6.5)
        plt.vlines(spemean, 0, 1e5, color='grey', linestyle='dashed', label='maximum = {}'.format(round(spemean,3)))
...
pd.DataFrame(results).transpose().to_csv('/media/disk_k/WbLS-DATA/csv/non-validated/phase8/bnl1t_spe_fit_results_'+date+'.csv',index=False)
#pd.DataFrame(results).transpose().to_csv('/home/juseong/temp/data/csv/bnl1t_spe_fit_results_'+date+'.csv',index=False)
...
for n,group in enumerate(['all','upper','lower','middle','bottom']):
    ...
    plt.vlines(spemean,0,1.1*np.max(ydata),color='black',linestyle='dashed')
