...
def create_plot(pivoted, output_file, title_suffix=""):
    ...
    X-axis tick labels are rotated 90 degrees and y-axis is fixed from 0.5 to 2.0.
    """
    ...
    for i, ch in enumerate(channels):
        ...
        # Add vertical line for April 1
        april_1 = datetime(2025, 4, 1)
        ax.axvline(april_1, color='red', linestyle='--', linewidth=1)
...
def main():
    ...
    # 두 개의 패턴을 리스트로 정의합니다.
    patterns = [
        re.compile(r"bnl30t_spe_fit_results_(\d{6})(?:_adjusted)?\.csv$"),
        re.compile(r"bnl30t_injection_spe_fit_(\d{6})(?:_(\d+))?(?:_adjusted)?\.csv$")
    ]
    ...
    for DATA_DIR in DATA_DIRS:
        ...
        all_files = sorted(
            ...
            if any(pattern.match(f) for pattern in patterns) and os.path.isfile(os.path.join(DATA_DIR, f))
        )
